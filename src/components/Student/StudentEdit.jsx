...
import { currencies, inputTypeDateValue, paymentRecurrenceType, studentStatus } from 'helpers'
...
export const CreateStudent = () => {
  ...
  const [formBtnText, setformBtnText] = useState('Update')
  ...
  useEffect(() => {
    Axios.get(`/api/student/edit/${studentID}`)
      .then(res => {
        ...
        setBirthdate(inputTypeDateValue(student.birthdate) || '')
        setCountry(student.country || '')
        setSuburb(student.suburb || '')
        setZipcode(student.zipcode || '')
        setJoinedDate(inputTypeDateValue(student.payment.joinedDate) || '')
        setAmount(student.payment.amount || '')
        setDeposit(student.payment.deposit || '')
        setQuantity(student.payment.quantity || '')
        setPaymentType(student.payment.paymentType || '')
        setPaymentTag(student.payment.paymentTag || '')
        setContractSigned(student.payment.contractSigned || '')
        setCurrency(student.payment.currency || '')
        setSalesGuy(student.payment.salesGuy || '')
        setPaymentMethod(student.payment.paymentMethod || '')
        setPaymentStatus(student.payment.paymentStatus || '')
        setPaymentTypeAmount(student.payment.paymentTypeAmount || '')
      })
    return () => {}
  }, [studentID])
  ...
  const handleSubmit = useCallback((e) => {
    ...
    if (
      ...
      paymentTypeAmount !== ''
        ...
        Axios.post(`/api/student/update/${studentID}`, {studentInfo, paymentInfo,})
          .then(res => {
            setformBtnText('Redirecting...')
            setDisableSubmitBtn(true)
            setAlertMsg(true)
            setTimeout(() => history.push(`/student/${res.data._id}`), 1500)
          })
          .catch(err => {
            setAlertMsg(false)
  ...
  return (
    <BaseLayout>
      ...
      <h2 style={{marginTop: '20px', marginBottom: '20px'}}>Edit Student</h2>
